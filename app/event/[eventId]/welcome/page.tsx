...
export default function WelcomePage({
  ...
  params: { eventId?: string; eventID?: string };
}) {

  const eventId = params.eventId ?? (params as any).eventID ?? "demo";
      ...
      <div className="min-h-screen w-full bg-gradient-to-b from-[#000A3B] to-[#001370] text-slate-100 flex items-center justify-center px-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-slate-300">
            ...
            <span className="font-mono text-slate-200">{params.eventId}</span>
          </p>
        </div>
      </div>
    );
